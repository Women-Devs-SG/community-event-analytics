...
export async function loadData(adapter: DataSourceAdapter): Promise<DashboardData> {
  ...
  return {
    ...
    satByEvent,
    refs: {
      medianDemand: median(events.map((event) => event.demand_index)),
      medianSatisfaction: median([...satByEvent.values()]),
    },
    source,
