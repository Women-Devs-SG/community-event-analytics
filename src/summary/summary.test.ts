...
const scopeOf = (summary: DashboardSummary, selection = ALL_EVENTS) => summary.scopeData[summary.scopes[scopeKey(selection)]];
...
function surveyRows(eventId: string, count: number, satisfaction: number) {
  ...
  for (let index = 0; index < count; index += 1) {
    ...
    responses.push({ response_id: responseId, event_id: eventId, satisfaction, recommend: satisfaction, experience_segment: '2-3 years' });
    feedback.push({ response_id: responseId, event_id: eventId, question_role: 'positive', text: `${eventId} comment ${index}` });
  }
...
function smallAndLargeEvent(): RawDataBundle {
  ...
  const datasets: RawDatasets = {
    events: [
      { event_id: 'large', event_name: 'Large event', event_date: '2026-01-10', format: 'Talk', topic_primary: 'Data', registered: 30, attended: 20, year_median_registered: 20 },
      { event_id: 'small', event_name: 'Small event', event_date: '2026-02-10', format: 'Talk', topic_primary: 'Data', registered: 5, attended: 3, year_median_registered: 20 },
    ],
  ...
  return {
    datasets,
    source: {
      sourceLabel: 'Test', sourceKind: 'test', dataClassification: 'synthetic', fetchedAt: new Date('2026-03-01T00:00:00Z'),
      reportingTimezone: 'UTC', contractVersion: 1, historyStart: null, historyEnd: null, limitations: [],
    },
  };
}
...
describe('build-time summary', () => {
  ...
  it('has a summary for every drill-down the effectiveness chart can open', async () => {
    ...
    for (const selection of reachableSelections(summary.events)) {
      ...
      for (const point of quadrant.topic.points) expect(summary.scopes[scopeKey({ ...selection, topic: point.id })]).toBeDefined();
      for (const point of quadrant.format.points) expect(summary.scopes[scopeKey({ ...selection, format: point.id })]).toBeDefined();
      for (const point of quadrant.event.points) expect(summary.scopes[scopeKey({ ...ALL_EVENTS, eventId: point.id })]).toBeDefined();
    }
  });
...
describe('Google Sheets adapter', () => {
  const tabs = { events: 'events', surveyResponses: 'survey', feedbackAnswers: 'feedback', registrations: 'registrations', participants: 'participants' };
  ...
  async function serviceAccountKey() {
    ...
    return { client_email: 'reader@example.iam.gserviceaccount.com', private_key: `-----BEGIN ${'PRIVATE KEY'}-----\n${body}\n-----END ${'PRIVATE KEY'}-----\n` };
  }
  ...
  it('reads a private sheet through the Sheets API with a service account', async () => {
    ...
    const adapter = createGoogleSheetsAdapter({
      sheetId: 'sheet-1', tabs, sourceLabel: 'Sheet', reportingTimezone: 'UTC',
      serviceAccountKey: await serviceAccountKey(), fetch: fetchMock as typeof fetch,
    });

    const bundle = await adapter.load();
    expect(bundle.datasets.events).toEqual([{ event_id: 'e1', event_name: 'Meetup' }, { event_id: 'e2', event_name: '' }]);
  ...
  it('warns when falling back to the public link, and explains a private sheet', async () => {
    const warn = vi.fn();
    const fetchMock = vi.fn(async () => new Response('<html>Sign in</html>', { headers: { 'content-type': 'text/html' } }));
    const adapter = createGoogleSheetsAdapter({
      sheetId: 'sheet-1', tabs, sourceLabel: 'Sheet', reportingTimezone: 'UTC', onWarning: warn, fetch: fetchMock as typeof fetch,
    });
