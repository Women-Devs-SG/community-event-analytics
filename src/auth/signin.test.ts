...
const validClaims = { aud: CLIENT_ID, iss: 'https://accounts.google.com', exp: String(NOW + 600), email: 'Organiser@Example.com', email_verified: 'true' };
...
describe('Apps Script access checks', () => {
  ...
  it('turns sheet values into rows keyed by header, skipping blank rows', () => {
    expect(script.rowsFromValues_([['event_id', 'event_name', ''], ['e1', 'Meetup', 'x'], ['', '', ''], ['e2']])).toEqual([
...
const asDisplayedText = (rows: RawRow[]) => rows.map((row) => Object.fromEntries(Object.entries(row).map(([key, value]) => [key, value == null ? '' : String(value)])));
...
describe('google-signin data loading', () => {
  it('sends the token in the request body as a simple request', async () => {
    ...
    await createAppsScriptAdapter({ url: 'https://script.example/exec', idToken: 'token-abc', sourceLabel: 'Sheet', reportingTimezone: 'UTC', fetch: fetchMock as typeof fetch }).load();
  ...
  it('builds the dashboard summary from rows returned to a signed-in viewer', async () => {
    ...
    const datasets = Object.fromEntries(Object.entries(synthetic.datasets).map(([key, rows]) => [key, asDisplayedText(rows)]));
    const fetchMock = vi.fn(async () => Response.json({ ok: true, datasets, fetchedAt: '2026-09-24T00:00:00Z' }));

    const data = await loadData(createAppsScriptAdapter({ url: 'https://script.example/exec', idToken: 't', sourceLabel: 'Sheet', reportingTimezone: 'UTC', fetch: fetchMock as typeof fetch }));
  ...
  it('explains refusals from the sheet backend', async () => {
    const fetchMock = vi.fn(async () => Response.json({ ok: false, code: 'not_authorized', message: 'x is not on the list' }));
    const load = createAppsScriptAdapter({ url: 'https://script.example/exec', idToken: 't', sourceLabel: 'Sheet', reportingTimezone: 'UTC', fetch: fetchMock as typeof fetch }).load();

    const error = await load.catch((reason: unknown) => reason);
    expect(error).toBeInstanceOf(SignInDataError);
    expect(formatSignInError(error, 'stranger@example.com')).toMatch(/stranger@example\.com doesn't have access.*share the reporting sheet/);
  });

  it('reports a misconfigured backend URL instead of failing silently', async () => {
    const fetchMock = vi.fn(async () => new Response('<html>Page not found</html>', { headers: { 'content-type': 'text/html' } }));
    await expect(
      createAppsScriptAdapter({ url: 'https://script.example/wrong', idToken: 't', sourceLabel: 'Sheet', reportingTimezone: 'UTC', fetch: fetchMock as typeof fetch }).load(),
...
describe('ID token display', () => {
  it('reads the signed-in name and email for display', () => {
    const payload = btoa(String.fromCharCode(...new TextEncoder().encode(JSON.stringify({ email: 'ana@example.com', name: 'Ána Tan' }))))
